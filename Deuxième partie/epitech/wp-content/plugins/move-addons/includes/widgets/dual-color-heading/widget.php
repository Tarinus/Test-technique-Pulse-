    ...
    protected function render( $instance = [] ) {
        ...
        if( !empty( $heading ) ){
            ...
            if ( $settings['heading_highlight_txt_pos'] == 'before' ) {
                $heading_text = sprintf( '<%1$s %4$s %2$s>%5$s %3$s</%1$s>', $heading_html_tag, $this->get_render_attribute_string( 'heading_attr' ), $heading, $heading_pl_txt, $highlight_txt );
            }else{
                $heading_text = sprintf( '<%1$s %4$s %2$s>%3$s %5$s</%1$s>', $heading_html_tag, $this->get_render_attribute_string( 'heading_attr' ), $heading, $heading_pl_txt, $highlight_txt );
            }
        }
